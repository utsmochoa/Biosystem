...
def obtener_ultimo_admin():
    """
    Obtiene el ID del último administrador registrado
    """
    conexion = conectar_bd()
    if conexion:
        try:
            cursor = conexion.cursor()
            query = "SELECT id FROM users WHERE rol = 'admin' ORDER BY id DESC LIMIT 1"
...
def obtener_admin_por_id(admin_id):
    ...
    if conexion:
        try:
            cursor = conexion.cursor()
            query = "SELECT id, name, rol FROM users WHERE id = %s AND rol = 'admin'"
...
def verificar_huella_duplicada_admin(template_bytes, admin_actual_id):
    """
    Verifica si la huella capturada ya existe en la base de datos usando FTRVerifyN
    Compara contra todas las huellas de usuarios (administradores y personal)
    """
    conexion = conectar_bd()
    if not conexion:
    ...
    try:
        ...
        query = """
            ...
            WHERE hu.users_id != %s
        """
        cursor.execute(query, (admin_actual_id,))
        ...
        return comparar_huellas_con_sdk_admin(template_bytes, huellas_existentes)
        
    except Error as e:
        print(f"Error al verificar duplicados: {e}")
        return False, None, None

def comparar_huellas_con_sdk_admin(template_bytes, huellas_existentes):
    """
...
def guardar_huella_admin(admin_id, template_bytes, calidad):
    """
    Guarda la huella del administrador en la base de datos
    """
    conexion = conectar_bd()
    if not conexion:
        return False
    try:
        ...
        cursor.execute(query, (admin_id, template_bytes, calidad))
        conexion.commit()
        return True
    except Error as e:
...
class AdminFingerprintGUI:
    def __init__(self, admin_id=None):
        ...
        # Si se proporcionó un ID, obtener los datos del administrador
        if self.admin_id:
            self.admin_data = obtener_admin_por_id(self.admin_id)
            if not self.admin_data:
                result = {
                    "success": False,
                    "message": f"No se encontró el administrador con ID {self.admin_id}"
                }
    ...
    def setup_window(self):
        self.root.title("Registrar Huella Administrador")
    ...
    def create_widgets(self):
        ...
        title_label = Label(main_frame, 
                           text="Registro de Huella Administrador", 
                           font=('Arial', 14, 'bold'),
                           bg='#f0f0f0',
                           fg='#2c3e50')
        ...
        if self.admin_data:
            info_text = f"Administrador: {self.admin_data[1]}\nID: {self.admin_data[0]}"
        else:
            info_text = "Nuevo administrador"
    ...
    def capture_fingerprint(self):
        ...
        try:
            ...
            admin_id = self.admin_id if self.admin_id else obtener_ultimo_admin()
            if not admin_id:
            ...
            es_duplicada, user_existente_id, datos_usuario = verificar_huella_duplicada_admin(
                template_bytes, 
                admin_id
            )
            ...
            if guardar_huella_admin(admin_id, template_bytes, eData.dwquality):
                # Registrar el reporte
                registrar_reporte(
                    user_id=admin_id,
                    tipo_accion="registro",
                    descripcion=f"Administrador registrado exitosamente desde lector FS88 para el usuario {admin_id}."
                )
                
                success = True
                message = f"Huella registrada correctamente para el administrador {admin_id}. Calidad: {eData.dwquality}"
            else:
...
def main():
    try:
        ...
        app = AdminFingerprintGUI(admin_id)
        app.run()
    except Exception as e:
        result = {
            "success": False,
