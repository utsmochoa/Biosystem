...
def obtener_ultimo_operador():
    """
    Obtiene el ID del último personal de seguridad registrado
    """
    conexion = conectar_bd()
    if conexion:
        try:
            cursor = conexion.cursor()
            query = "SELECT id FROM users WHERE rol = 'operador' ORDER BY id DESC LIMIT 1"
            cursor.execute(query)
            operador = cursor.fetchone()
            cursor.close()
            conexion.close()
            return operador[0] if operador else None
        except Error as e:
            return None
    return None

def obtener_operador_por_id(operador_id):
    """
    Obtiene los datos de un personal de seguridad específico por su ID
    """
    conexion = conectar_bd()
    if conexion:
        try:
            cursor = conexion.cursor()
            query = "SELECT id, name, rol FROM users WHERE id = %s AND rol = 'operador'"
...
def verificar_huella_duplicada_operador(template_bytes, operador_actual_id):
    """
    Verifica si la huella capturada ya existe en la base de datos usando FTRVerifyN
    Compara contra todas las huellas de usuarios (personal de seguridades y personal)
    """
    conexion = conectar_bd()
    if not conexion:
    ...
    try:
        cursor = conexion.cursor()
        # Obtener todas las huellas existentes de usuarios excepto el actual
        query = """
            SELECT hu.users_id, hu.huella_data, u.name, u.rol 
            FROM huellas_users hu
            JOIN users u ON hu.users_id = u.id
            WHERE hu.users_id != %s
        """
        cursor.execute(query, (operador_actual_id,))
        huellas_existentes = cursor.fetchall()
        cursor.close()
        conexion.close()
        
        print(f"Verificando contra {len(huellas_existentes)} huellas existentes de usuarios...")
        ...
        return comparar_huellas_con_sdk_operador(template_bytes, huellas_existentes)
        
    except Error as e:
        print(f"Error al verificar duplicados: {e}")
        return False, None, None

def comparar_huellas_con_sdk_operador(template_bytes, huellas_existentes):
    """
    Compara usando FTRVerifyN del SDK para usuarios
    """
    try:
        ...
        for i, (user_id, huella_existente, nombre, rol) in enumerate(huellas_existentes):
            print(f"Comparando con huella {i+1}/{len(huellas_existentes)} del usuario {user_id} ({nombre} - {rol})")
            ...
            try:
                ...
                if res == FTR_RETCODE_OK:
                    if result.value == 1:  # Match found
                        print(f"¡DUPLICADO ENCONTRADO! Usuario {user_id} ({nombre} - {rol}), FARN: {ftr_farn.value}")
                        return True, user_id, {'name': nombre, 'rol': rol}
                    else:
                        print(f"No coincide con usuario {user_id}, FARN: {ftr_farn.value}")
                else:
                    print(f"FTRVerifyN falló con código: {res} para usuario {user_id}")
                    
            except Exception as e:
                print(f"Error comparando huella del usuario {user_id}: {e}")
                continue
...
def guardar_huella_operador(operador_id, template_bytes, calidad):
    """
    Guarda la huella del personal de seguridad en la base de datos
    """
    conexion = conectar_bd()
    if not conexion:
        return False
    try:
        cursor = conexion.cursor()
        query = "INSERT INTO huellas_users (users_id, huella_data, quality) VALUES (%s, %s, %s)"
        cursor.execute(query, (operador_id, template_bytes, calidad))
        conexion.commit()
        return True
    except Error as e:
...
def registrar_reporte(user_id, tipo_accion, descripcion):
    """
    Registra un reporte en el historial de usuarios
    """
    conexion = conectar_bd()
    if conexion:
        try:
            cursor = conexion.cursor()
            query = """
                INSERT INTO historial_usuarios (users_id, tipo_accion, descripcion, fecha_hora)
                VALUES (%s, %s, %s, NOW())
            """
            valores = (user_id, tipo_accion, descripcion)
            cursor.execute(query, valores)
            conexion.commit()
            print("[INFO] Reporte registrado.")
        except Error as e:
...
class operadorFingerprintGUI:
    def __init__(self, operador_id=None):
        self.operador_id = operador_id
        self.operador_data = None
        
        # Si se proporcionó un ID, obtener los datos del personal de seguridad
        if self.operador_id:
            self.operador_data = obtener_operador_por_id(self.operador_id)
            if not self.operador_data:
                result = {
                    "success": False,
                    "message": f"No se encontró el personal de seguridad con ID {self.operador_id}"
                }
    ...
    def setup_window(self):
        self.root.title("Registrar Huella personal de seguridad")
    ...
    def create_widgets(self):
        ...
        title_label = Label(main_frame, 
                           text="Registro de Huella personal de seguridad", 
                           font=('Arial', 14, 'bold'),
                           bg='#f0f0f0',
                           fg='#2c3e50')
        title_label.pack(pady=(0, 10))
        
        # Información del personal de seguridad
        if self.operador_data:
            info_text = f"personal de seguridad: {self.operador_data[1]}\nID: {self.operador_data[0]}"
        else:
            info_text = "Nuevo personal de seguridad"
    ...
    def capture_fingerprint(self):
        ...
        try:
            ...
            operador_id = self.operador_id if self.operador_id else obtener_ultimo_operador()
            if not operador_id:
                message = "No se pudo obtener el ID del personal de seguridad"
                ftrdll.FTRTerminate()
                self.show_result(success, message)
                return
            ...
            es_duplicada, user_existente_id, datos_usuario = verificar_huella_duplicada_operador(
                template_bytes, 
                operador_id
            )
            ...
            if es_duplicada:
                if datos_usuario:
                    message = f"Esta huella ya está registrada para el usuario: {datos_usuario['name']} ({datos_usuario['rol']})"
                else:
                    message = f"Esta huella ya está registrada para otro usuario (ID: {user_existente_id})"
            ...
            if guardar_huella_operador(operador_id, template_bytes, eData.dwquality):
                # Registrar el reporte
                registrar_reporte(
                    user_id=operador_id,
                    tipo_accion="registro",
                    descripcion=f"personal de seguridad registrado exitosamente desde lector FS88 para el usuario {operador_id}."
                )
                
                success = True
                message = f"Huella registrada correctamente para el personal de seguridad {operador_id}. Calidad: {eData.dwquality}"
            else:
...
def main():
    try:
        ...
        if len(sys.argv) > 1:
            try:
                operador_id = int(sys.argv[1])
            except ValueError:
                result = {
                    "success": False,
                    "message": "ID de personal de seguridad inválido"
                }
                print(json.dumps(result))
                sys.exit(1)
        
        app = operadorFingerprintGUI(operador_id)
        app.run()
    except Exception as e:
        result = {
            "success": False,
